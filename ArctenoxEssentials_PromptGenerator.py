...
def generate_prompt(
    ...
    prompt = ", ".join(parts).replace("_", " ").replace("(", chr(92) + "(" ).replace(")", chr(92) + ")")
...
# ─────────────────────────────────────────────────────────────────────────────
#  Node — TagNormalizer
#         Deduplicates and normalises a comma-separated tag string
# ─────────────────────────────────────────────────────────────────────────────

def _normalize_tag(tag: str) -> str:
    """Lowercase, collapse whitespace, unify spaces/underscores → underscore."""
    return re.sub(r"\s+", "_", tag.strip().lower())


def _normalize_and_dedup(text: str, preserve_order: bool) -> tuple:
    """
    Split on commas, normalise each tag, remove duplicates.
    Returns (cleaned_string, removed_count, original_count).
    """
    ...
    for tag in raw_tags:
        ...
        if key not in seen:
            seen.add(key)
            # Keep the original casing/spacing from the first occurrence
            result.append(tag)

    if not preserve_order:
        # Sort: pure-underscore / booru-style tags alphabetically,
        # hand-written tags (contain spaces) kept but still deduped
        result.sort(key=lambda t: _normalize_tag(t))
...
class TagNormalizer:
        ...
        return {
            "required": {
                "text": ("STRING", {
                    "multiline": True,
                    "default":   "",
                    "tooltip":   "Comma-separated tag string to clean.",
                }),
                "preserve_order": ("BOOLEAN", {
                    "default": True,
                    "tooltip": "Keep tags in original order. Disable to sort alphabetically.",
                }),
                ...
                "underscores_to_spaces": ("BOOLEAN", {
                    "default": True,
                    "tooltip": "Convert tag_underscores to spaces in the output.",
                }),
            },
        }
    ...
    DESCRIPTION = (
        "Deduplicates and normalises a comma-separated tag string. "
        "Removes duplicate tags regardless of case or underscore/space differences. "
        "Optionally escapes parentheses and converts underscores to spaces."
    )
    ...
    def normalize(
        ...
        cleaned, removed, original = _normalize_and_dedup(text, preserve_order)

        if underscores_to_spaces:
            cleaned = cleaned.replace("_", " ")

        if escape_parentheses:
            cleaned = cleaned.replace(chr(92) + "(", "(").replace(chr(92) + ")", ")").replace("(", chr(92) + "(").replace(")", chr(92) + ")")

        debug = (
            f"Input tags:   {original}\n"
            f"Output tags:  {original - removed}\n"
