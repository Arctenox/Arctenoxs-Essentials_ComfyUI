...
# Load VAE With Metadata
try:
    from .ArctenoxEssentials_LoadVAE import LoadVAEWithMetadata
except ImportError as e:
    print(f"[Arctenox Essentials] Warning: Could not import Load VAE: {e}")
    LoadVAEWithMetadata = None
...
NODE_CLASS_MAPPINGS = {
    ...
    "ArctenoxLoadVAE":                      LoadVAEWithMetadata,
    # Latent
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "ArctenoxLoadVAE":                      "Load VAE (Arctenox's Essentials)",
    # Latent
...
def _print_welcome():
    ...
    print(f"\n  New in v1.3.0:")
    for item in [
        "CLIP Text Encode [Positive + Wildcards]  — positive-only with wildcard support",
        "CLIP Text Encode [Prompt Gen]  — appends PromptGenerator output to positive",
        "Wildcard String  — multiline string primitive with wildcard resolution",
        "Tag Normalizer  — deduplicates tags, escapes parentheses, underscore→space",
        "Prompt Generator  — filter_tags, exclude artist/copyright, min_post_count step 1",
        "Prompt Finalizer  — coherence-based pruning & reordering via co-occurrence data",
        "Wildcard Nodes  — Processor, Picker, Stack (fixed & stable)",
    ]:
        print(f"    + {item}")
    print("=" * 62 + "\n")
